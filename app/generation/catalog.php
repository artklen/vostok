...
d()->catalog_seo_params = [
	'h1' => function($object) {
		return d()->catalog_seo_data['h1'];
	},
	'title' => function($object) {
		return d()->catalog_seo_data['title'];
	},
	'description' => function($object) {
		return d()->catalog_seo_data['description'];
	},
	'canonical' => function($object) {
		return isset(d()->catalog_seo_data['canonical']) ? d()->catalog_seo_data['canonical'] : '';
	},
];

/*
 * d()->this_products - список товаров
 * d()->products__fields - свойства товаров в фильтре
 * d()->get - значения свойств товаров на текущей странице (необязательно)
 */
d()->singleton('catalog_seo_data', function() {
	$result = [];
	$catalog_url = '/catalog';
	...
	if (!empty($special_title_arr)) {
		...
		$result['h1'] = $catalog_full_title . (!empty($title_arr) ? ' ' . implode(' ', $title_arr) : '');
	}
